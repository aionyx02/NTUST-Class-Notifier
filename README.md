...
- **限流會自己退讓** — 所有查詢共用一條連線（keep-alive），遇到 429 / 5xx / 連線失敗會照 `Retry-After` 或指數退避重試最多 3 次；退避時間同時記成冷卻，下一輪的第一個請求會先把冷卻等完，等於自動放慢輪詢。系所名額查詢例外：它一門課要打一次請求，冷卻中直接跳過、失敗也不重試，免得幾十門課把整輪拖上好幾分鐘。
- **系所名額檢查** — 寫了 `系所:…` 時會查該課程的保留名額，擋掉「總數有空位但你的系已滿」的假空位；命中超過 50 門則跳過（那支 API 一門課要一次請求）。被限流冷卻期間也會跳過並在 log 出聲——那一輪可能把系所已額滿的假空位當成真的。
- **時段精確到分鐘** — 選課時段的日期寫在 `core/periods.py`，並以 `Asia/Taipei` 判斷到分鐘：只比日期的話，開放首日的凌晨就會開始送、最後一天收攤後還會送到午夜。日期沒到、當天還沒開放（09:00–17:00 之外）、或內建時程整份都已過期，一律只監控不加選。
- **自動加選分兩個階段** — 電選課走 `First/A06/ExtraJoin`、全校加退選走 `AddAndSub/B01/ExtraJoin`，由目前時段決定；兩邊的已選清單格式不同，驗證方式也各自一套（加退選讀 `#cartTable`）。
- **不會重複送出加選** — 實測時對「已在選課清單裡」的課再送一次 `ExtraJoin`，那門課就從清單消失了。所以送出前一定先確認課程不在清單中，已選上的一律略過；搶到之後空位就算還在也不會再送，補滿後再次釋出才會重新開始。
- **失敗的那一門還會再試** — 網路斷一下、session 剛好過期、名額被別人搶走，都只是「這次沒成功」。只要空位還在，下一輪會再送一次，同一次空位期間最多 3 次；選課系統當天收攤後狀態整個歸零，隔天開放是全新的機會。
- **每次送出前都自己確認一次清單** — 電選課擋重送用的清單不含志願序，所以送出前一定先讀一次含志願序的清單，已經在裡面就不送；**清單讀不到時也不送**（不知道在不在志願序裡，就不能冒著把它取消的風險送出去）。
- **一輪最多加選 5 門** — 規則寫太廣（例如 `課號:CS`）時可能一次湧出幾十門空位，連續送出幾十次加選正是學校明文禁止的行為，超過就整批不送並提示你縮小規則（提示一次空位潮只講一次；空位降回 5 門以內就自動恢復加選）。
- **session 自己顧** — 登入後每 3 分鐘碰一次選課頁面（看的是「今天屬於哪一段」，所以選課期間的半夜也照碰，早上九點才不會發現已被登出）；選課系統的登入過期時不會回錯誤碼，而是回一個登入頁，所以是看頁面上有沒有登出表單來判斷，失效就地重新登入。監看與維持 session 跑在同一個事件迴圈，等空位等上幾小時也不會醒來才發現已被登出。
...
**一次 push 只跳一級**，跳哪一級看這批 commit 裡最大的那一種：

| 這批 commit 裡有 | 版本變化 |
| --- | --- |
| 任何一個 `feat: …` | 次版號 +1、修訂號歸零 |
| 只有 `fix: …` | 修訂號 +1 |
| 其他（`docs:`、`chore:`…） | 不發布 |

例如 `0.1.6` 之後推上 2 個 feat 加 3 個 fix，發的是 `0.2.0`；只有 3 個 fix
就是 `0.1.7`。次版號到 `0.9.0` 再加一個 feat 是 `0.10.0`，**主版號永遠不會
自動跳**。
