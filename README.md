...
只在課程「額滿 → 有空位」的瞬間提醒（大字綠色橫幅＋音效），空位持續存在不會重複洗版，名額被補滿時補一行紅字。**只提醒，不會替你加選。**
...
- **永遠只有一則訊息** — 每次更新都先刪掉舊訊息再送新的，聊天室不會被洗版。
...
3. **遇到 429 / Too Many Requests？** — 請求太頻繁被限流，拉長 `-i` 即可；規則涵蓋太多課程時週期也會自動放寬。
