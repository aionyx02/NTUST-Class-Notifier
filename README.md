...
- **課程空額監控** — 透過 `querycourse.ntust.edu.tw` API 並發查詢多門課程的目前人數 / 上限，每輪偵測人數變動。
- **Discord 通知** — 設定 Bot Token 與收件對象後，人數一有變動就更新通知；對象填 ID 即可，伺服器 / 頻道 / 使用者會自動判斷。
...
程式會依環境變數啟動對應功能：讀取 `LOOK_UP_CLASSES` 監控空額；有 Discord 變數則發送通知。以 `Ctrl+C` 中止。

Discord 通知的行為：

- **永遠只有一則訊息** — 每次更新都先刪掉舊訊息再送一則新的狀態，不會愈積愈多。
- **人數一有變動就更新** — 不是只看「有沒有空位」：任何一門課被選走或有人退選都會馬上重送看板，並在下方列出這一輪的變動（`-` 紅色被選走、`+` 綠色有人退選）。完全沒有變動時每 5 分鐘也會重送一次，讓訊息上的時間戳保持新鮮。
...
- **`.env` 同名變數不能寫兩行**（只有最後一行有效）；要分行寫請用 `LOOK_UP_CLASSES_1`、`_2`…，程式會自動合併。
